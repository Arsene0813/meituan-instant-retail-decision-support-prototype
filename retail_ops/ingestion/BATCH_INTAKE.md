...
这一步的 `validated` 表示接入校验通过。SQL、fact、API 和 RAC 仍读取现有项目文件；它们尚未切换到批次库。后续发布层需要选择明确批次，生成不可变快照，并让一次分析从头到尾使用同一个发布版本。
...
批次库暂不提供自动“最新版本”查询或分析发布命令。下一步将用两版同店同窗口数据及缺值附表，验证一次发布及其 SQL、fact、RAC 的共同版本选择。
