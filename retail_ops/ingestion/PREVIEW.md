...
规范 CSV 的原文留存、人工核对的来源登记、重复接收和修订链现已接入本地批次库，见 [批次留存与修订](BATCH_INTAKE.md)。真实后台认证、其他来源格式与单位映射、分析版本发布及 SQL/RAC 从批次读取仍待接通。当前预览通过表示本模块的检查通过。

四份现有 SQL 已接入规范数值校验，并检查 Top 3 和平均值所需的数据是否完整，详见 [SQL 数值与聚合处理](../sql/README.md)。真实批次仍需通过事务发布明确的版本，SQL 和 RAC 从这些版本读取数据。本预览只检查同一次上传中的重复主键；批次库另外校验重试和显式修订。跨来源及跨数据集的重叠选择仍需在发布层验证。
