...
入口：`python3 -m retail_ops.ingestion.preview`。输入已核对的规范 CSV，输出 JSON 预览。上传批次在这里限定为一个已确认的门店、实际来源窗口和数据集。包含多个门店或窗口的文件需要先按来源区块拆分，再分别传入各自的上传上下文。
...
- 来源记录的门店和日期必须与上传上下文一致。原 `canonical_csv_v1` 保持完整自然月规则；`canonical_csv_v2` 支持已明确登记的实际来源窗口，规则见 [SOURCE_QUERY.md](SOURCE_QUERY.md)。
...
在项目根目录运行。`--input` 应指向包含单一店铺、单一来源窗口的规范 CSV。确认的门店、期间及类型由操作人或上传配置提供。
...
输出仅到终端。全部通过时退出码为 0；需要核对时为 2。实际窗口的独立预览使用 `--mapping-version canonical_csv_v2`；持久化接收时解析规则来自受核对的上传登记。可选 `--proposals` 读取 JSON 数组，每项必须包含 `dataset_id`、`grain`、`ranking_basis` 和 `record`，顺序与 CSV 数据行一致。
...
规范 CSV 的原文留存、人工核对的来源登记、重复接收和修订链现已接入本地批次库，见 [批次留存与修订](BATCH_INTAKE.md)。[发布入口](PUBLICATION.md) 可以明确选择批次，并让 SQL、Demo 2 fact 和 RAC 从同一快照读取；真实后台认证、其他来源格式与单位映射及 API 切换仍待接通。当前预览通过表示本模块的检查通过。

四份现有 SQL 已接入规范数值校验，并检查 Top 3 和平均值所需的数据是否完整，详见 [SQL 数值与聚合处理](../sql/README.md)。发布入口重放校验所选批次，完整生成视图与结果后公布版本。本预览只检查同一次上传中的重复主键；批次库另外校验重试和显式修订。发布层按已登记的重叠范围要求选择一个完整来源版本。
